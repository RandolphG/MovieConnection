...
export const useDetails = () => {
  useEffect(() => {
    /* Variables */
    const modalContainer: HTMLElement = <HTMLElement>(
      document.querySelector(".modal")
    );

    const openButton: HTMLElement = <HTMLElement>(
    ...
    const volumeButton: HTMLElement = <HTMLElement>(
    ...
    const isSilent = (): boolean => volume == 1;

    function volumeHandler() {
      if (isSilent()) {
        /* If the volume is equal to 0, activate it and put it in LocalStorage. */
        volume = 1;
