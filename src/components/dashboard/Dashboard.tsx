...
function Dashboard() {
  ...
  const handleSubmit = (e: any) => {
    switch (value) {
      case "Low rating":
        setMovies(moviesList.sort((a, b) => (a.rating > b.rating ? 1 : -1)));
        break;
      case "High rating":
        setMovies(moviesList.sort((a, b) => (b.rating > a.rating ? 1 : -1)));
        break;
      case "A-Z":
        setMovies(
          moviesList.sort((a, b) => (strip(a.title) > strip(b.title) ? 1 : -1))
        );
        break;
      case "Z-A":
        setMovies(
          moviesList.sort((a, b) => (strip(b.title) > strip(a.title) ? 1 : -1))
        );
        break;
      default:
  ...
  return (
    <div>
      ...
      <section>
        <MovieTemplate movies={moviesList} />
      </section>
    </div>
  );
}
